 ...
 */
$required_class = 'PHP_CodeSniffer_CommentParser_ClassCommentParser';
...
class moodle_Sniffs_Commenting_FileCommentSniff implements PHP_CodeSniffer_Sniff {
    ...
    public function process(PHP_CodeSniffer_File $phpcsfile, $stackptr) {
        ...
        // Find the first doc block.
        $filedoctoken = $phpcsfile->findnext(T_DOC_COMMENT, ($stackptr + 1));
        ...
        if ($tokens[$filedoctoken]['code'] === T_CLOSE_TAG) {
            ...
            if (empty($long) === false) {
                ...
                if ($newlinebetween !== 2) {
                    $error = 'There should be exactly one blank line between'
                           .' descriptions in file comment';
            ...
            if (method_exists($this, $method) === true) {
                ...
                if (is_array($tagelement) === true) {
                    ...
                    foreach ($tagelement as $key => $element) {
                     ...
                     $tagelement->process($this->currentfile, $commentstart, $this->docblock);
                }
            }
        }
     ...
     *
     * Accepts either PHP_CodeSniffer_CommentParser_Singleelement or
     * PHP_CodeSniffer_CommentParser_Pairelement.
     *
     * @param string $tagname    The name of the doc comment element.
     * @param object $tagelement The doc comment element.
     * @return void
     */
    ...
    protected function processpackage($errorpos) {
        ...
        if ($package !== null) {
            ...
            if (empty($CFG->dirroot)) {
                $warning = "Unable to find moodle install directory.  Unable to validate package name \"$content\".";
                $this->currentfile->addwarning($warning, $errorpos);
            } else if ($content !== $expected_package) {
        ...
        if (empty($authors) === false) {
            ...
            foreach ($authors as $author) {
                ...
                if ($content !== '') {
                    ...
                    $localmiddle = $local.'.\w';
                    $regex = '/^([^<]*)\s+<(['.$local.']['.$localmiddle.']*['
                           .$local.']@[\da-zA-Z][-.\w]*[\da-zA-Z]\.[a-zA-Z]{2,7})>$/';

                    if (preg_match($regex, $content) === 0) {
                        $error = 'content of the @author tag must be in the'
                               .' form "Display name <username@example.com>"';
                        $this->currentfile->adderror($error, $errorpos);
                    }
    ...
    protected function processcopyrights($commentstart) {
        ...
        foreach ($copyrights as $copyright) {
            ...
            if ($content !== '') {
                ...
                if (preg_match($regex, $content, $matches) !== 0) {
                    // Check earliest-latest year order.

                    if ($matches[3] !== '') {

                        if ($matches[3] !== '-') {
                            $error = 'A hyphen must be used between the'
                                   .' earliest and latest year';
                            $this->currentfile->adderror($error, $errorpos);
                        }

                        if ($matches[4] !== '' && $matches[4] < $matches[1]) {
                            $error = 'Invalid year span "'. $matches[1]
                                   . $matches[3] . $matches[4]
                                   .'" found; consider "'. $matches[4] .'-'
                                   . $matches[1] .'" instead';
                    ...
                    $error = '@copyright tag must contain a year and the'
    ...
    protected function processlicense($errorpos) {
        ...
        if ($license !== null) {
            ...
            if ($value === '' || $comment === '') {
                $errors[] = '@license tag must contain a URL and a license name';
            }

            if ($comment != 'GNU GPL v3 or later') {
                $errors[] = 'License must be "GNU GPL v3 or later", found "'
                          . $comment .'"';
            }

            if ($value != 'http://www.gnu.org/copyleft/gpl.html') {
                $errors[] = 'License url must be "http://www.gnu.org/copyleft/'
                          .'gpl.html", found "'.$value.'"';
            }
        ...
        if ($version !== null) {
            ...
            if (empty($content) === true) {
                ...
                $error = "Invalid version \"$content\" in file comment;"
                       .' consider "CVS: <cvs_id>" or "SVN: <svn_id>" instead';
                $this->currentfile->addwarning($error, $errorpos);
            }
        }
